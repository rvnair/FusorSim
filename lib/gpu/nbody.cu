...
static constexpr int GOOD_BLOCK_SIZE = 64;
...
__global__
static void compute_forces(real_v* q0, double* c, real_v* out, uint64 N)
{
  ...
  int big = floor((sqrt((float)8*id +1)-1)/2);
  int lil = id - big*(big+1)/2;
  int q0i = big;
  int q1i = lil;
  if(q1i == q0i) q1i = N;
  real_v to_out = force_part(q0[q0i].vals, q0[q1i].vals, c[q0i], c[q1i]);
...
static void qdot1_update(real_v* qdot0, real_v* Fq1, double dt, uint64 N, double* m)
{
  ...
  if(id < N)
  {
    qdot0[id].vals[blockIdx.y] += dt * Fq1[id].vals[blockIdx.y]/m[id];
  }
}
...
struct comp_Fq1
{
  ...
  {
    real_v ret;
    for(int i = 0; i < REAL_V_DIM; i++) ret.vals[i] = a.vals[i] + b.vals[i];
    return ret;
  }
};
...
static void step(real_v* q, real_v* qdot, double* m, double* c, double dt, uint64 N, real_v* out, real_v* Fq1, real_v* temp_q, const cudaStream_t s[streams], void (*passed)(output), output prepped)
{
  ...
  //Update q1
  q1_update<<<q1_blocks, q1_thread,0,s[0]>>>(q, qdot, dt, N);
  cudaEventRecord(event, s[0]);
  cudaStreamWaitEvent(s[1], event, 0);
...
void n_body_eval(real_v* q, real_v* v, double* m, double* c, double QE, double r, double dt, uint64 n, uint64 N, void (*passed)(output))
{
  ...
  cudaMemsetAsync(&dev_q[N], 0, sizeof(real_v), s[5]);
  cudaDeviceSynchronize();
  prep.numb = N;
  prep.part = nullptr;
  ...
  {
    prep.step = i;
    step(dev_q, dev_v, dev_m, dev_c, dt, N, d_out, d_Fq1, q, s, passed, prep);
  }
  cudaFree(dev_q);
  cudaFree(dev_v);
  cudaFree(dev_m);
